...
const MissionCategoryIcon: React.FC<{ category: Mission['category'] }> = ({ category }) => {
    const styles = {
        ...
        Soul: { bg: 'bg-purple-100', text: 'text-purple-600', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg> }
    };
    const style = styles[category] || styles.Mind;
    return <div className={`flex-shrink-0 p-3 rounded-full ${style.bg} ${style.text}`}>{style.icon}</div>;
}

const DifficultyBadge: React.FC<{ difficulty: Mission['difficulty'] }> = ({ difficulty }) => {
    ...
    return (
        <span className={`text-xs font-semibold ml-2 px-2.5 py-0.5 rounded-full ${styles[difficulty]}`}>
            {difficulty}
        </span>
    );
};
...
const MissionItem: React.FC<MissionItemProps> = ({ mission, onComplete, isCompletable }) => {
    ...
    const containerClasses = mission.isActivationMission
        ? 'bg-yellow-50 border-yellow-400 border-2'
        : mission.isCompleted ? 'bg-slate-100' : 'bg-white border border-slate-200';

    return (
        <div className={`p-4 rounded-lg flex items-center transition-colors duration-300 ${containerClasses}`}>
